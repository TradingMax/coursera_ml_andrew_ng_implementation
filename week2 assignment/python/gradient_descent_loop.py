...
def cost_function(x_arr: list[float], y_arr: list[float], w1: float, w2: float) -> float:
    running_sum = 0
...
def derivative_cost_function_w1(x_arr: list[float], y_arr: list[float], w1: float, w2: float):
    running_sum = 0
    for x, y in zip(x_arr, y_arr):
...
def derivative_cost_function_w2(x_arr: list[float], y_arr: list[float], w1: float, w2: float):
    running_sum = 0
    for x, y in zip(x_arr, y_arr):
...
def run_gradient_descent(x_arr: list[float], y_arr: list[float], w1_init: float, w2_init: float, a: float):
    tolerance = 1e-16
    w1, w2 = w1_init, w2_init
    d1 = d2 = 1
    i = 0
    while abs(d1) > tolerance and abs(d2) > tolerance:
        print(i := i + 1)
...
if __name__ == '__main__':
    ...
    w1_result, w2_result = run_gradient_descent(x_test, y_test, 0, 0, 0.05)

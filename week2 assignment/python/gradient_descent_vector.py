...
def run_gradient_descent(x: np.ndarray, y: np.ndarray, w_init: np.ndarray, a: float):
    tolerance = 1e-16
    max_iter = 1500
    w = w_init
    d = np.ones((len(w),))
...
if __name__ == '__main__':
    ...
    x_test = np.vstack([np.ones(len(x_test)), x_test])  # Add a row of ones to x_test
    w_test = run_gradient_descent(x_test, y_test, np.array([0.1, 0.1]), 0.01)
